...
def load_data():
    item_train = genfromtxt("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_item_train.csv", delimiter=',')
    user_train = genfromtxt("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_user_train.csv", delimiter=',')
    y_train    = genfromtxt("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_y_train.csv", delimiter=',')
    with open("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_item_train_header.txt", newline='') as f:
        item_features = list(csv.reader(f))[0]
    with open("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_user_train_header.txt", newline='') as f:
        user_features = list(csv.reader(f))[0]
    item_vecs = genfromtxt("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_item_vecs.csv", delimiter=',')
       
    movie_dict = defaultdict(dict)
    count = 0
    with open("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_movie_list.csv", newline='') as csvfile:
    ...
    with open("D:\ML\From scratch\Recommender system\Content-based filtering RecSys\Data\content_user_to_genre.pickle", 'rb') as f:
...
def predict_uservec(user_vecs, item_vecs, model, u_s, i_s, scaler, ScalerUser, ScalerItem, scaledata=False):
    ...
    if scaledata:
        ...
        y_pu = model.predict([scaled_user_vecs[:,u_s:],scaled_item_vecs[:,i_s:]], verbose = 0)
        y_pu = scaler.inverse_transform(y_pu)
    else:
        y_pu = model.predict([user_vecs[:,u_s:],item_vecs[:,i_s:]], verbose = 0)
        
    sort_index = np.argsort(y_pu, axis = 0)[::-1]
    y_pu = y_pu[sort_index[:,0]]
    sorted_user = user_vecs[sort_index[:,0]]
    sorted_item = item_vecs[sort_index[:,0]]
    return(y_pu, sorted_user, sorted_item)
